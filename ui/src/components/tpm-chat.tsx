...
const PROSE = [
  ...
  'prose-li:my-0',
  'prose-pre:my-1.5 prose-pre:rounded-md prose-pre:text-xs prose-pre:leading-relaxed',
  'prose-code:text-xs prose-code:bg-muted prose-code:px-1 prose-code:py-px prose-code:rounded prose-code:font-mono prose-code:before:content-none prose-code:after:content-none',
  ...
  'prose-table:text-xs prose-th:font-medium prose-th:py-1 prose-td:py-1',
...
function ToolChip({ tc, onInspect }: { tc: ToolCall; onInspect: (tc: ToolCall) => void }) {
  return (
    <button
      ...
      className={cn(
        'inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-mono font-medium border transition-all',
        tc.status !== 'running' && 'hover:scale-105 active:scale-95 cursor-pointer',
  ...
  return (
    <div
      className={cn(
        ...
        'md:flex md:shrink-0 md:self-stretch md:h-[100dvh] md:overflow-hidden transition-all duration-300',
        isOpen ? 'w-full md:w-auto' : 'w-0 md:w-0 md:overflow-hidden',
        // Mobile: fixed full-height drawer when open, hidden otherwise
        isOpen ? 'fixed inset-0 z-50 flex flex-col md:flex-row md:static md:z-auto shadow-2xl md:shadow-none bg-background md:bg-transparent' : 'hidden md:flex',
      )}
    >
      {/* Artifact panel */}
      <div className={cn(
        'flex flex-col bg-card border-l border-border transition-all duration-300 shrink-0 overflow-hidden self-stretch',
        artifactOpen && hasStories ? 'w-full md:w-96 flex-1' : 'w-0 border-l-0 h-0 md:h-auto',
      )}>
            ...
            <div className="flex items-center gap-1 overflow-x-auto scrollbar-none flex-1 min-w-0">
              {mergedStories.map((s, idx) => (
                <button key={s.filename} onClick={() => setActiveTab(idx)}
                  className={cn(
                    'flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap transition-colors shrink-0 border',
          ...
          <div className="flex-1 overflow-auto select-text">
            {activeStory ? (
              <div className="p-4">
                <div className="flex items-center justify-between mb-3 gap-2">
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground font-mono truncate">{activeStory.filename}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    {!savedStories.has(activeStory.filename) ? (
                      <Button size="sm" variant="outline" className="h-6 px-2 text-xs gap-1" onClick={() => saveStory(activeStory)} disabled={saving}>
                ...
                <div className="rounded-md border border-border bg-zinc-950 dark:bg-slate-950 overflow-hidden">
                  <pre className="p-3 text-xs leading-relaxed text-slate-200 whitespace-pre-wrap font-mono overflow-x-auto">{activeStory.yaml}</pre>
                </div>
              </div>
            ) : (
      ...
      {/* Main chat */}
      <div className="flex flex-col bg-background border-t md:border-t-0 md:border-l border-border shrink-0 w-full md:w-96 self-stretch flex-1 min-h-0">
        <div className="w-full md:w-96 flex-1 flex flex-col min-h-0">
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            {hasStories && (
              <Button variant={artifactOpen ? 'secondary' : 'ghost'} size="icon" className="h-6 w-6 relative" onClick={() => setArtifactOpen(!artifactOpen)} aria-label="Toggle artifact panel">
                ...
                <span className="absolute -top-0.5 -right-0.5 flex h-3 w-3 items-center justify-center rounded-full bg-primary text-primary-foreground text-xs font-bold border border-background">{mergedStories.length}</span>
              </Button>
            )}
            {hasStories && !allSaved && !streaming && (
              <Button size="sm" className="h-6 px-1.5 text-xs gap-0.5 font-semibold" onClick={saveAll} disabled={savingAll}>
        ...
        {/* Messages */}
        <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto px-4 py-4 [scrollbar-width:thin]">
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center h-full gap-5 text-center select-none">
              ...
              {activeProject && (
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-muted/40 border border-border/50 text-xs w-full max-w-48">
              ...
              {messages.map((msg, i) => (
                <div key={i} className={cn('animate-in fade-in duration-200', msg.role === 'user' ? 'flex flex-col items-end' : '')}>
                  {msg.role === 'user' ? (
                    ...
                    <div className="bg-muted text-foreground px-3 py-2 text-sm leading-snug whitespace-pre-wrap font-medium tracking-tight max-w-4/5 rounded-2xl rounded-br-sm">
                      {msg.content}
                    </div>
                  ) : (
                    ...
                    <div className="group">
                      ...
                      {/* Markdown */}
                      {msg.content && (
                        <div className={cn(PROSE, 'text-foreground text-sm')}>
                      ...
                      {i === lastAsstIdx && !streaming && msg.tokenCount && (
                        <div className="flex items-center gap-1 mt-1.5 ml-0.5">
                          ...
                          <span className="text-xs tabular-nums text-muted-foreground/50">~{msg.tokenCount < 1000 ? msg.tokenCount : `${(msg.tokenCount / 1000).toFixed(1)}k`}</span>
                          <span className="text-xs tabular-nums text-muted-foreground/40">· {((msg.durationMs || 0) / 1000).toFixed(1)}s</span>
                          {(msg.tokensPerSec || 0) > 0 && <span className="text-xs tabular-nums text-emerald-500/50">· {msg.tokensPerSec}/s</span>}
        ...
        {!isEmpty && !streaming && (
          <div className="px-3 pb-1 flex gap-1 flex-wrap shrink-0">
            {QUICK.map(q => (
              <button key={q.label} onClick={() => handleSend(q.msg)}
                className="text-xs font-medium px-2 py-0.5 rounded-full border border-border bg-muted/30 hover:bg-muted text-muted-foreground hover:text-foreground transition-all">
                {q.label}
              </button>
            ))}
          </div>
        ...
        {/* Input — cowork-style glassmorphic, rounded-3xl */}
        <div className="px-3 pb-4 pt-2 bg-background/40 backdrop-blur-xl shrink-0 border-t border-border/40">
          <div className="relative flex flex-col bg-background/70 backdrop-blur-md border border-border/60 shadow-sm focus-within:border-foreground/25 focus-within:ring-1 focus-within:ring-foreground/10 transition-all rounded-2xl">
            {/* @mention popover */}
            {mentionQuery && filteredMentions.length > 0 && (
              <div ref={mentionRef}
                className="absolute bottom-full mb-1.5 left-0 right-0 bg-background/95 backdrop-blur-xl border border-border/50 shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-bottom-2 duration-150 rounded-xl">
                <div className="px-2.5 py-1 border-b border-border/30 text-xs uppercase tracking-wider text-muted-foreground/50 font-bold bg-muted/20 flex items-center gap-1">
                ...
                <div className="max-h-40 overflow-y-auto p-0.5 space-y-px">
                  {filteredMentions.map((m, idx) => (
                    <button key={m.id} onClick={() => insertMention(m)}
                      ...
                      <span className="text-xs truncate font-medium flex-1">{m.label}</span>
                      {m.status && (
            ...
            <div className="flex items-center justify-between px-3 pb-2 pt-0">
              <div className="flex items-center gap-3">
                <ModelSelector />
                <p className="text-[10px] text-muted-foreground/30 font-medium tracking-wide">⏎ send · ⇧⏎ new</p>
              </div>
      ...
      <Dialog open={!!selectedTool} onOpenChange={() => setSelectedTool(null)}>
        <DialogContent className="flex flex-col max-h-full w-full sm:max-w-lg border border-border p-0 bg-background shadow-2xl rounded-2xl">
          <DialogHeader className="border-b border-border px-4 py-2.5 shrink-0 flex flex-row items-center justify-between space-y-0">
            <DialogTitle className="text-xs font-semibold flex items-center gap-1.5 font-mono">
              {selectedTool && <>
                <span className={cn('w-2 h-2 rounded-full shrink-0', selectedTool.status === 'success' ? 'bg-emerald-500' : 'bg-destructive')} />
                {selectedTool.name}
                {selectedTool.duration && <span className="text-xs text-muted-foreground/50 font-normal">{(selectedTool.duration / 1000).toFixed(2)}s</span>}
              </>}
            </DialogTitle>
          ...
          {selectedTool && (
            <div className="flex-1 overflow-auto p-3 space-y-2.5 min-h-0">
              {selectedTool.arguments && Object.keys(selectedTool.arguments).length > 0 && (
                <div>
                  <p className="text-xs uppercase tracking-wider text-muted-foreground/50 font-bold mb-1">Input</p>
                  <pre className="text-xs text-foreground/70 bg-muted/30 rounded-lg border border-border/30 p-2.5 overflow-x-auto whitespace-pre-wrap font-mono">{JSON.stringify(selectedTool.arguments, null, 2)}</pre>
                </div>
              )}
              {selectedTool.result && (
                <div>
                  <p className="text-xs uppercase tracking-wider text-muted-foreground/50 font-bold mb-1">Output</p>
                  <pre className="text-xs text-foreground/70 bg-muted/30 rounded-lg border border-border/30 p-2.5 overflow-x-auto whitespace-pre-wrap font-mono select-text max-h-96 overflow-y-auto">{selectedTool.result}</pre>
                </div>
              )}
            </div>
          )}
