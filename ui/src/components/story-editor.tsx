...
const parseYamlFields = (yaml: string) => {
  ...
  const agentModelMatch = yaml.match(/^agentModel:\s*["']?([^"'\n]+)["']?/m);
  ...
  return {
    ...
    agentModel: agentModelMatch ? agentModelMatch[1] : '',
  };
};
...
const updateYamlField = (yaml: string, field: string, value: any): string => {
  ...
  } else if (field === 'agentModel') {
    if (yaml.match(/^agentModel:/m)) {
      if (!value) {
        // remove it if empty
        updated = yaml.replace(/^agentModel:\s*.*$\n?/m, '');
      } else {
        updated = yaml.replace(/^agentModel:\s*.*$/m, `agentModel: ${value}`);
      }
    } else if (value) {
      updated = yaml + `\nagentModel: ${value}`;
    }
...
export function StoryEditor({ storyFile, storyName, onClose, onSaved, onViewLogs, onStopBuild }: StoryEditorProps) {
  ...
  const [availableModels, setAvailableModels] = useState<{id: string, displayName: string, provider: string}[]>([]);
    ...
    fetch('/api/settings')
      ...
      .then(data => {
        if (data.providers) {
          ...
          const mappedModels = enabledProviders.map((p: any) => {
            const modelId = p.defaultModel || p.kind;
            const modelObj = (p.models || []).find((m: any) => m.id === modelId);
            const modelName = modelObj ? (modelObj.name || modelObj.id) : modelId;
            return {
              id: `${p.id}:::${modelId}`,
              displayName: modelName,
              provider: p.name
            };
          }).filter((m: any) => m.id && !m.id.endsWith(':::'));
          setAvailableModels(mappedModels);
        }
      })
      .catch(err => console.error('Failed to fetch settings for models', err));
  }, []);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <select
                value={parsedFields.agentModel || ''}
                onChange={(e) => updateField('agentModel', e.target.value)}
                className="h-9 w-[140px] px-2 py-1 text-xs font-medium rounded-md border border-border bg-muted/50 text-foreground hover:bg-muted focus:ring-1 focus:ring-ring focus:outline-none truncate"
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Button variant="outline" size="icon" onClick={handleCopy} className="shrink-0 bg-background">
                <Copy className="h-3.5 w-3.5 text-muted-foreground hover:text-foreground" />
              </Button>

              {isDirty && (
                <Button variant="outline" size="icon" onClick={handleReset} className="shrink-0 text-rose-600 dark:text-rose-400 bg-background hover:bg-rose-500/5 hover:text-rose-500 border-border">
              ...
              <Button
                ...
                className="px-4 text-xs gap-1.5 font-semibold shrink-0"
              >
